namespace MiHoYoAssets.Formats
{
    public class Pack : Format
    {
        ...
        {
            Name = "PACK";
            DisplayName = "Genshin Impact - Distency";
            Pattern = ("*.blk", "");
            Extension = (".unity3d", "");
        }
        ...
        protected override void Decrypt(string input, string output)
        {
            ...
            while (reader.Position != reader.Length)
            {
                ...
                signature = reader.ReadStringToNull(UnityFSSignature.Length);
                if (signature == UnityFSSignature)
                {
                    var tempData = reader.ReadBytes(MaxEntrySize - 0x80);
                    var UnityFSBlock = tempData.Search(UnityFSSignature);
