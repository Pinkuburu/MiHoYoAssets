    ...
    public class GI : Format
    {
        ...
        private string[] CollectDirectories(string input)
        {
            ...
            foreach (var item in dirs)
            {
                ...
                if (files.Length == 0 || files.Any(x => !x.Name.Contains("CAB-")))
                {
                    continue;
                }
        ...
        protected override void Decrypt(string input, string output)
        {
            ...
            var blk = new Blk(ref reader, ExpansionKey, ConstKey, Const, SBox);
            var buffer = blk.Decrypt(ref reader);
            ...
            try
            {
                while (mhy0Reader.Position != mhy0Reader.Length)
                {
                    ...
                    var mhy0 = new Mhy0(ref mhy0Reader);
                    mhy0.WriteFiles(output);
                }
            }
            catch (Exception ex)
            {
        ...
        protected override void Encrypt(string input, string output)
        {
            ...
            var files = dir.GetFiles("CAB-*", SearchOption.TopDirectoryOnly);
            var cabs = files.Select(x => Path.GetFileNameWithoutExtension(x.Name)).Distinct().ToList();
            using var mhy0Stream = new MemoryStream();
            foreach (var cab in cabs)
            {
                var entries = files.Where(x => x.Name.Contains(cab)).ToArray();
