namespace MiHoYoAssets.Containers
{
    public class Mhy0
    {
        ...
        public Mhy0(ref EndianReader reader)
        {
        ...
        public Mhy0(FileInfo[] files)
        {
            BlocksStream = CreateBlocksStream(files);
            ReadFiles(files);
            WriteNodes();
            WriteBlocks();
        }
        ...
        {
            var header = WriteBlocksInfoAndDirectory();
            WriteMetadata(header, mhy0Stream);
        }
        ...
        private void WriteMetadata(byte[] header, MemoryStream mhy0Stream)
        {
            header = CompressAndWriteHeader(header);
            ScrambleHeader(header);

            var buffer = new byte[CalculateHeaderSize(header)];
            var writer = new EndianWriter(buffer);
            writer.WriteString(Signature);
            writer.WriteUInt8Array(header);
            writer.Write(BlocksStream.GetBuffer(), 0, TotalCompressedSize);
            BlocksStream.Dispose();
            mhy0Stream.Write(buffer);
        }

        private int CalculateHeaderSize(byte[] header) => TotalCompressedSize + Signature.Length + 4 + header.Length;
        private int CalculateUncompressedHeaderSize(int bundleCount, int blockCount) => 0x113 * bundleCount + 0xD * blockCount + 0xC;

        private static byte[] CompressAndWriteHeader(byte[] header)
        {
            var uncompressedSize = header.Length;
            var uncompressed = header;
            ...
            var buffer = new byte[compressedSize + 0x27];
            var writer = new EndianWriter(buffer);
            writer.WriteUInt32(mhy0HeaderMagic);
        ...
        {
            var bundleCount = DirectoryInfo.Length;
            var blockCount = BlocksInfo.Length;

            var buffer = new byte[CalculateUncompressedHeaderSize(bundleCount, blockCount)];
            var writer = new EndianWriter(buffer);
            writer.WriteMhy0Int2(bundleCount);
            ...
            writer.WriteMhy0Int2(blockCount);
        ...
        private MemoryStream CreateBlocksStream() => new(TotalUncompressedSize);

        private MemoryStream CreateBlocksStream(FileInfo[] files)
        {
            var uncompressedSizeSum = files.Sum(x => (int)x.Length);
        ...
        private void WriteBlocks()
        {
            ...
            while (totalUncompressedSize > 0)
            {
                ...
                Array.Copy(compressedBytes, 0, compressedBytes, 0xC, compressedSize);
                var mhy0EntrySize = compressedSize + 0xC;
                var writer = new EndianWriter(compressedBytes);
                writer.WriteUInt32(mhy0EntryMagic++);
                writer.WriteInt64(Random.Shared.NextInt64());
                ScrambleEntry(compressedBytes);

                BlocksStream.Position = writePos;
                BlocksStream.Write(compressedBytes, 0, mhy0EntrySize);
                writePos = BlocksStream.Position;
                BlocksStream.Position = readPos;
        
                var blockInfo = new StorageBlock() { UncompressedSize = uncompressedSize, CompressedSize = mhy0EntrySize };
        ...
        {
            DirectoryInfo = new Node[FileList.Length];
            for (int i = 0; i < FileList.Length; i++)
            {
                ...
                var node = new Node
                {
                    ...
                    IsAssetFile = !Path.HasExtension(file.FileName),
                    Offset = BlocksStream.Position
                };

                file.Stream.CopyTo(BlocksStream, node.Size);
                file.Stream.Dispose();

                DirectoryInfo[i] = node;
            }
        ...
        private void ReadFiles(FileInfo[] filesInfo)
        {
            FileList = new StreamFile[filesInfo.Length];
            for (int i = 0; i < filesInfo.Length; i++)
            {
                ...
                var file = new StreamFile
                {
                    FileName = info.Name,
                    Path = info.Name,
                    Stream = new MemoryStream(File.ReadAllBytes(info.FullName))
                };
                FileList[i] = file;
            }
        }
