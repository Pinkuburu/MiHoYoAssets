namespace MiHoYoAssets.Containers
{
    public static class Mark
    {
        ...
        public static byte[] Decrypt(ref EndianReader reader)
        {
            ...
            var bufferSize = CalculateSize(reader.Remaining, true);
            ...
            {
                var blockSize = Math.Min(reader.Length - reader.Position, BlockSizeWithPadding);
        ...
        public static byte[] Encrypt(byte[] inBuffer)
        {
            ...
            var bufferSize = Signature.Length + CalculateSize(inBuffer.Length);

            XORBlock(inBuffer);
            var outBuffer = new byte[bufferSize];
            var writer = new EndianWriter(outBuffer);
        ...
        private static int CalculateSize(int size, bool hasPadding = false)
        {
            var blockCount = size / (hasPadding ? BlockSizeWithPadding : BlockSize);
            var remainingCount = size % (hasPadding ? BlockSizeWithPadding : BlockSize);
            var chunkCount = remainingCount / (hasPadding ? ChunkSizeWithPadding : ChunkSize);

            int bufferSize;
            if (hasPadding)
            {
                bufferSize = size - ((BlockPadding * blockCount) + (ChunkPadding * chunkCount));
            }
            else
            {
                bufferSize = size + (BlockPadding * blockCount) + (ChunkPadding * chunkCount);
